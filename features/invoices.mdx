...
description: "Issue invoices and credit notes from within Google Sheets. Each document is saved to MySQL, adjusts lot inventory, and generates a linked Google Sheets invoice file."
---
...
<Steps>
  <Step title="Open the invoice dialog">
    Go to **Arsinous menu → Create Invoice** or click **New Invoice** in the sidebar. This calls `showInvoice(null, "invoice")`, which opens a 1000 × 600 px Vuetify modal. On load, the modal fetches all customers, products, in-stock inventory lots (`irs_items WHERE in_stock > 0`), salesperson records, and customer discounts from MySQL in parallel.
  </Step>
...
**6. Update the invoice spreadsheet content**

`updateInvoiceSs(ssId, data)` opens the invoice Google Sheets file and writes customer code, date, invoice number, salesperson, and the full items grid (columns A–O, rows 15–54).

**7. Commit the transaction**

`conn.commit()` finalises all database changes. The connection is then closed before the Drive and Sheets operations run.
