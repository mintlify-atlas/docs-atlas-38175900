---
title: "Arsinous V8 Sales: Google Sheets Sales Platform"
sidebarTitle: "Introduction"
description: "Arsinous V8 Sales is a Google Apps Script system connecting Google Sheets to MySQL for end-to-end sales management across customers, invoices, and inventory."
---
...
**Apps Script backend** — All server-side logic lives in `.gs` files organised by domain (`Customer/`, `product/`, `invoice/`, `Payment/`, `DB/`, `sidebar/`, plus root-level utility scripts). Google Apps Script executes these files server-side in V8 (see [Setup](/setup) for the current runtime note).
...
function onOpen(e) {
  ...
  ui.createMenu("Arsinous")
    ...
    .addSeparator()
    .addItem("GET Discounts", "getDiscounts")
    .addItem("SAVE Discounts", "saveDiscounts")
    .addToUi();
...
Arsinous V8 Sales targets the **Google Apps Script V8 runtime** (Chrome V8 engine), enabling modern JavaScript syntax throughout the codebase. See the [Setup guide](/setup) for the current `appsscript.json` runtime status and migration note.
